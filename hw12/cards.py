...
Card -- a single card from a deck of cards
     __init__(self, face=None, suit=None) 
...
# Useful constants
# These are defined BEFORE the classes so that they are
# easily accessible to both classes and outside users
...
class Card:
    ...
    # Tuples for suit and face names
    # The face name tuple contains two empty elements
    ...
    def __init__(self, face=None, suit=None):
        ...
        Default arguments of face=None and suit=None create 
            random card.
    ...
    def __str__(self):
        ...
        return '%s of %s' \
            % (Card.__FACE_LIST[self.__face], 
            Card.__SUIT_LIST[self.__suit])
    
class DeckOfCards:
    """Represent a deck of cards."""

    def __init__(self):
        """Create a deck containing all cards in random order."""
        self.__card_list = []
    ...
    def is_empty(self):
        """Return whether deck is empty or not.""" 
        return len(self.__card_list) == self.__top
...
def main():
    # Simple demo program to demonstrate a few deck and card methods
    print('This program demonstrates the deck constructor')
    print('and a few card methods.\n')

    deck = DeckOfCards()
    for i in range(5):
        current_card = deck.deal()
        print('Card %i = %s' % (i + 1, current_card))
        if current_card.is_ace():
            print('   The card is an ace')
        elif current_card.is_face_card():
            print('   The card is a face card')
        face = current_card.get_face()
        print('   The face value is %d.' % face)
